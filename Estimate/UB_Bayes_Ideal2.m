...
data_dir = 'D:\Documents\MATLAB\SNS\predict_data';

S_ZZ = load(fullfile(data_dir, 'thetaR_ZZ_MAP_results_7.mat'));
S_XX = load(fullfile(data_dir, 'thetaR_XX_MAP_results_7.mat'));
...
for i = 1:length(S_ZZ.L_values)
    ...
    S_z = epsilon^2 * S_s + (1-epsilon)^2 * S_0 + epsilon*(1-epsilon)*S_ua_0 + epsilon*(1-epsilon)*S_0_ub;
    E_z = (epsilon^2 * S_s + (1-epsilon)^2 * S_0) / S_z;
