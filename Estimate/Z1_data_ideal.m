function [s1, e1, P] = Z1_data_ideal(signal_A, decoy_A, signal_B, decoy_B, pd, pc, alpha, d, e_det)
sA = signal_A;
sB = signal_B;
...
%----------------------------------------------------------
S_s = 1 - (1-pd)^2*exp(-2*sA*eta_trans);
S_s_half = 1 - (1-pd)^2*exp(-sA*eta_trans);
S_d1 = 1 - (1-pd)^2*exp(-2*A1*eta_trans);
S_d2 = 1 - (1-pd)^2*exp(-2*A2*eta_trans);
S_0 = 1 - (1-pd)^2;

%----------------------------------------------------------
E_s = 0.5 - 1/(2*S_s) * (1-pd) * ...
     (exp(-2*sA*eta_trans*e_det) - ...
     exp(-2*sA*eta_trans*(1-e_det)));

E_d1 = 0.5 - 1/(2*S_d1) * (1-pd) * ...
     (exp(-2*A1*eta_trans*e_det) - ...
     exp(-2*A1*eta_trans*(1-e_det)));

E_d2 = 0.5 - 1/(2*S_d2) * (1-pd) * ...
     (exp(-2*A2*eta_trans*e_det) - ...
     exp(-2*A2*eta_trans*(1-e_det)));
