function [s1, e1, P] = Z1_data_Bayes2(signal_A, decoy_A, signal_B, decoy_B, pd, pc, alpha, d, e_det)
sA = signal_A;
sB = signal_B;
...
%----------------------------------------------------------
S_d1 = 1 - (1-pd)^2*exp(-(A1+B1)*eta_trans);
S_d2 = 1 - (1-pd)^2*exp(-(A2+B2)*eta_trans);
S_0 = 1 - (1-pd)^2;

%----------------------------------------------------------
E_d1 = 0.5 - 1/(2*S_d1) * (1-pd) * ...
     (exp(-(A1+B1)*eta_trans*e_det) - ...
     exp(-(A1+B1)*eta_trans*(1-e_det)));
