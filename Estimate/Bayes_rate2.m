...
for i = 1:length(S_ZZ.L_values)
    ...
    [s1_Bayes, e1_Bayes, P] = Z1_data_Bayes2(signal_A, decoy_A, signal_B, decoy_B, pd, pc, alpha, d, e_det);

    eta_trans = 10^(-alpha*d/10)*pc;

    S_0 = 1 - (1-pd)^2;

    S_ua_0 = 1 - (1-pd)^2*exp(-signal_A*eta_trans);
    S_0_ub = 1 - (1-pd)^2*exp(-signal_B*eta_trans);
    S_s_half = S_ua_0 + S_0_ub;

    S_s = 1 - (1-pd)^2*exp(-(signal_A+signal_B)*eta_trans);
