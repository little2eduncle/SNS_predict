function check_gradients_XXwindow_click_prob_asym_screened()
% Finite-difference gradient check for:
%   [result, grad] = XXwindow_click_prob_asym_screened(...)
%
% It compares analytic gradients against finite differences
% for the CONDITIONAL accepted probabilities:
%   [P_only_D0, P_only_D1, P_rest]
%
% This script automatically switches to one-sided differences
% near parameter boundaries.
    ...
    % -------------------------------------------------------------
    [result0, grad0] = call_model_xx(p);

    fprintf('Base XX-screened conditional probabilities:\n');
    fprintf('  P_only_D0     = %.16g\n', result0.P_only_D0);
    fprintf('  P_only_D1     = %.16g\n', result0.P_only_D1);
    fprintf('  P_rest        = %.16g\n', result0.P_rest);
    fprintf('  Sum check     = %.16g\n', ...
        result0.P_only_D0 + result0.P_only_D1 + result0.P_rest);
    fprintf('  acceptFraction= %.16g\n\n', result0.acceptFraction);
    ...
    for i = 1:numel(param_names)
        ...
        h = choose_step_xx(x0, name);

        [p_plus, p_minus, ok, scheme] = make_fd_points_xx_check(p, name, h);
        if ~ok
            warning('Skip parameter %s because a valid FD step could not be constructed.', name);
            continue;
        end

        [r_plus,  ~] = call_model_xx(p_plus);
        [r_minus, ~] = call_model_xx(p_minus);
    ...
    % -------------------------------------------------------------
    fprintf('Gradient check results (XX screened)\n');
...
% =====================================================================
function [result, grad] = call_model_xx(p)
    [result, grad] = XXwindow_click_prob_asym_screened( ...
        p.muA, p.muB, ...
        p.pd0, p.pc0, p.pa0, ...
        p.pd1, p.pc1, p.pa1, ...
...
% =====================================================================
function h = choose_step_xx(x, name)
% Relative step with a minimum floor.
% For lambda we use a slightly larger minimum because the accepted region
% can change rapidly when lambda is very small.
...
% =====================================================================
function [p_plus, p_minus, ok, scheme] = make_fd_points_xx_check(p, name, h)
% Build FD points while respecting domains.
%
% scheme = 'central' or 'forward'
    ...
    switch name
        ...
        case 'lambda'
            % domain: (0,1]
            % Use forward difference near 0, backward-like one-sided near 1
            if x0 - h <= 0
                p_plus.(name)  = min(1, x0 + h);
                p_minus.(name) = x0;
                scheme = 'forward';
