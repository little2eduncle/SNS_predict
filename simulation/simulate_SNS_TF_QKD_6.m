function [C_total, C_windows, C_ZZ_intensity, C_XX_intensity] = ...
    simulate_SNS_TF_QKD_6(N, thetaA, thetaB, thetaC, varargin)
% simulate_SNS_TF_QKD
%
% 功能:
...
for chunk = 1:num_chunks
    ...
    % 3) 相位差
    if use_gpu
        phase_A = 2 * pi * rand(1, chunk_size, 'gpuArray');
        phase_B = 2 * pi * rand(1, chunk_size, 'gpuArray');
    else
        phase_A = 2 * pi * rand(1, chunk_size);
        phase_B = 2 * pi * rand(1, chunk_size);
    end
    ...
    muA_after_loss = muA .* pc0 .* etaA;
    muB_after_loss = muB .* pc0 .* etaB;

    % 6)  相干态构造
    state_A = sqrt(muA_after_loss) .* exp(1i*phase_A);
    state_B = sqrt(muB_after_loss) .* exp(1i*phase_B);

    % 7) 50:50 BS 干涉
    % T = 0.5;
    % BS = gpuArray([sqrt(T), -sqrt(1-T);
    %     sqrt(1-T), sqrt(T)]);
    % [a1_after_BS, a2_after_BS]' = BS * [a1, a2]'
    %                             = [(a1 + a2)/sqrt(2), (a1 - a2)/sqrt(2)]

    state_A_after_BS = (state_A + state_B) / sqrt(2);
    state_B_after_BS = (state_A - state_B) / sqrt(2);

    % 8) 点击概率
    I0 = abs(state_A_after_BS).^2;
    I1 = abs(state_B_after_BS).^2;
    ...
    % 8) 伯努利抽样
    if use_gpu
        D0_click = rand(1, chunk_size, 'gpuArray') < P0;
        D1_click = rand(1, chunk_size, 'gpuArray') < P1;
    else
        D0_click = rand(1, chunk_size) < P0;
        D1_click = rand(1, chunk_size) < P1;
    end

    outcome = 1 + double(D1_click) + 2 * double(D0_click);

    % 9) ZZ / XX 统计口径
    phase_ok = abs(phase_A-phase_B) >= (1 - lambda);
